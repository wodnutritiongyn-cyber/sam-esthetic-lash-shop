...
Deno.serve(async (req) => {
  ...
  const clearOtherPopups = async (excludeId?: string) => {
    const q = supabase.from("coupons").update({ show_in_popup: false }).eq("show_in_popup", true);
  ...
  try {
    ...
    if (action === "create") {
      ...
      if (body.show_in_popup) await clearOtherPopups();
    ...
    if (action === "update") {
      ...
      if (updates.show_in_popup) await clearOtherPopups(id);
